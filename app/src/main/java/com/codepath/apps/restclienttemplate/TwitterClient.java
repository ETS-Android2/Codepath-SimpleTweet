...
public class TwitterClient extends OAuthBaseClient {
	...
	public void getHomeTimeline(JsonHttpResponseHandler handler) {
		...
		//String apiUrl = getApiUrl("statuses/home_timeline_OOPS.json"); // to check we get a failure

		// Can specify query string params directly or through RequestParams.
		RequestParams params = new RequestParams();
		params.put("count", 25);
		params.put("since_id", 1); // every tweet will have an ID greater than 1, will modify
		                           // 	later to only get the most recent tweets
		client.get(apiUrl, params, handler);
	}

	// need to pass in a tweet's ID
	public void getNextPageOfTweets(JsonHttpResponseHandler handler, long maxId) {
		...
		RequestParams params = new RequestParams();
		params.put("count", 25);
		params.put("max_id", maxId); // every tweet will have an ID greater than 1, will modify
							     // 	later to only get the most recent tweets
		client.get(apiUrl, params, handler);
	}
